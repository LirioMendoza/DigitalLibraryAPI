...
class UserRoutes(Blueprint):
    ...
    def userRoutes(self):
        ...
        self.route('/api/profile/<int:user-id>', methods=['GET'])(self.getUser)
     ...
     def addUser(self):
        try:
            ...
            sel.password = self.data.get('password')
    ...
    def updateUser(self, user_id):
        try:
            ...
            try: # Validations for the data
                if self.user_name:
                    self.resource_schema.validateUser_name(self.user_name)

                if self.first_name:
                    self.resource_schema.validateFirst_name(self.first_name)

                if self.last_name:
                    self.resource_schema.validateLast_name(self.last_name)

                if self.email:
                    self.resource_schema.validateEmail(self.email)
                
                if self.password:
                    self.resource_schema.validatePassword(self.password)
